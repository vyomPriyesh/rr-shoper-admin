...
const ImagesUploadUi = ({
    ...
    const getFileInfo = (object) => {
        ...
        } else if (object?.fileType == 'pdf' || object?.fileType == 'excel' || object?.fileType == 'word') {
            newObject = {
                url: object?.viewUrl,
                image: object?.viewUrl,
            }
        }
        ...
        } else if (object?.fileType == 'excel') {
            colorIcon = {
                icon: FaFileExcel,
                color: '#16A34A'
            }
        } else if (object?.fileType == 'word') {
            colorIcon = {
                icon: FaFileWord,
                color: '#2563EB'
            }
    ...
    const { mutate: imageHandle, isPending } = useMutation({
        ...
        onSuccess: ({ data }) => {
            ...
            if (!multiple) {
                const image = getFileInfo(uploadedImages[0])

                onChange?.(image);
            } else {
                const imagesList = uploadedImages.map((img) => getFileInfo(img));
        ...
        <div>
            <Upload
                ...
                itemRender={(originNode, file) => {
                    ...
                    const { icon: Icon = FiFile, color, name } = getFileInfo(file);
